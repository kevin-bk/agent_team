...
export default defineConfig(({ mode }) => {
  ...
  // `.env` values are not on `process.env` here, so load them explicitly to
  // resolve the dev proxy target. Dev server proxies the API + SSE to the
  // FastAPI process (default :8765); override with VITE_API_PROXY. In prod the
  // built bundle is served by FastAPI itself, so the proxy is dev-only.
  const env = loadEnv(mode, process.cwd(), "");
  const API_TARGET = env.VITE_API_PROXY ?? "http://127.0.0.1:8765";
