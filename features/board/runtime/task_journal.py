...
def _render_journal_markdown(items: list[tuple]) -> str:
    """Render journal rows (ascending ``seq``) into a readable markdown timeline."""
    lines = [
        "# Task Journal",
        "",
        "Append-only timeline of this task's key decisions, assumptions, risks, "
        "approvals and verification outcomes. Read it for prior context; newest "
        "is last. (You do not edit this file — append suggestions to "
        "`.agent-team/JOURNAL_NOTES.jsonl` instead.)",
    ]
    for seq, created, actor_type, actor_id, phase, type_, title, body, severity in items:
        ts = created.isoformat() if created else ""
        actor = actor_type + (f"/{actor_id}" if actor_id else "")
        meta = " · ".join(p for p in (actor, severity, ts) if p)
        lines += ["", f"## #{seq} · {type_} · {phase}", f"_{meta}_", "", f"**{title}**"]
        if body:
            lines += ["", body]
    return "\n".join(lines) + "\n"


def write_journal_file(task_id: str, workspace_path: str) -> str | None:
    """Mirror the full durable journal to ``.agent-team/JOURNAL.md`` (best-effort).

    Regenerated each turn so an agent can read the complete decision history on
    demand (a light prompt pointer tells it where). Returns the workspace-relative
    path written, or ``None`` when there is nothing to write or on any failure.
    """
    if not workspace_path:
        return None
    db = None
    try:
        ...
        db = SessionLocal()
        rows = journal_repo.list_entries(db, task_id, limit=500)
        items = [
            (
                e.seq,
                e.created_at,
    ...
    try:
        _artifacts.write_text(
            workspace_path, _artifacts.JOURNAL_FILE_PATH, _render_journal_markdown(items)
        )
    except Exception:  # pragma: no cover - mirror is best-effort
        return None
    return _artifacts.JOURNAL_FILE_PATH
