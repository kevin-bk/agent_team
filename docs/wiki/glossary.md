...
- **Verification receipt** — backend-owned proof that one human-approved command
  actually ran in the task runtime, bound to its output hashes and exact source
  state. The evaluator may cite it but cannot mint it.
- **Verification evidence** — receipt references plus evaluator-observed
  scenarios/artifacts recorded in `EVIDENCE.json`; a `pass` without valid proof
  is downgraded to `fail` (`verdict.py`).
